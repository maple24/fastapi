...
# 0. log into RQM
RQMclient = CRQMClient("ets1szh", "estbangbangde4",
                       "GAC_A58A88", "https://rb-alm-20-p.de.bosch.com")
re = RQMclient.login()
print(f"Login {re}")
...
'''
ltcers_id = [RQMclient.getAllTCERSbasedonTestplanandTestcases(
    sTestplan_id, i) for i in lTestcases_id]
...
'''
ltcresults_id = list()
for i in zip(lTestcases_id, lTestcases_title, ltcers_id, lTestcases_status, lTestcases_stepresults):
    content = RQMclient.createExecutionResultTemplate(
        testcaseID=i[0], testcaseName=i[1],  TCERID=i[2], resultState=i[3], testplanID=sTestplan_id, stepResults=i[4], buildrecordID=br_id)
