...
class Aritifacoty_Download:
    ...
    def __init__(
        ...
        if self.dstfolder is None:
            ...
            self.dstfolder = os.path.join(
                os.path.dirname(os.path.abspath(__file__)), self.dstfolder
            )
        if not os.path.exists(self.dstfolder):
            os.mkdir(self.dstfolder)
        logging.basicConfig(
            level=logging.DEBUG,
            filename=os.path.join(self.dstfolder, "multithread_dl.log"),
        ...
        try:
            ...
            while True:
                ...
                if piece:
                    ...
                    read_times += 1
                    print(
                        "Downloading progress - [{}MB/{}MB]".format(
                            int(read_times), file_size
                        )
                    )
                else:
